...
export const updateArgWidths = (
    ...
    let blockWidth =
        numArgs * ArgsConfig.ARG_PADDING +
        (state[id].defaultBlockWidthLines || 0);
    if (args) {
        for (let i = 0; i < numArgs; i++) {
            if (args && args[i] === null) {
                ...
                state = {
                    ...updateArgWidths(state, args[i] as string, ignoredArgId),
                };
                blockWidth += state[args[i] as string].blockWidthLines;
                argWidths &&
                    (argWidths[i] = state[args[i] as string].blockWidthLines);
            }
        }
    }
